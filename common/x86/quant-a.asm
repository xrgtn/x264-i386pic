...
cglobal quant_%1x%2_dc, 3,3,8
    DESIGNATE_RPIC r1 ; r1m is used, r1 is not: designate it for no-save PIC
...
%if cpuflag(ssse3)
    ...
    mova      m5, [pic(chroma_dc_dmf_mask)]
%else
    mova      m3, [pic(chroma_dc_dct_mask_mmx)]
    mova      m5, [pic(chroma_dc_dmf_mask_mmx)]
%endif
    pshuflw   m2, m2, 0
...
%if HIGH_BIT_DEPTH
    ...
    pabsd     m0, m0
    PIC_BEGIN r4
    vpcmpud   k1, m0, [pic(pd_1)] {1to16}, 6
    PIC_END
%else
    vptestmw  k0, m0, m0
    pabsw     m0, m0
    PIC_BEGIN r4
    vpcmpuw   k1, m0, [pic(pw_1)], 6
    PIC_END
%endif
%endmacro
...
%else ; !ARCH_X86_64
    ...
    shr   edx, 1
    SHRX  edx, ecx
    %define table pic(decimate_table4)
%endif ; ARCH
    tzcnt ecx, edx
    shr   edx, 1
    SHRX  edx, ecx
...
%if HIGH_BIT_DEPTH
    ...
    jnz .ret9
    PIC_BEGIN r4
    mova            m1, [pic(decimate_shuf_avx512)]
    PIC_END
    vpermd          m0, m1, m0
    vptestmb        k1, m0, m0
%else
...
.loop:
    tzcnt ecx, r2
    add    al, byte [pic(decimate_table8) + ecx]
    jge .ret9
...
.end:
    ...
    PIC_END
    RET
    PIC_CONTEXT_POP ; make it look like PIC_END didn't happen
.tryret:
    BLSR   r2, r3
    jz .ret
    mov    al, -6
...
%macro LAST_MASK_AVX2 2 ; PIC*[HIGH_BIT_DEPTH]
%if HIGH_BIT_DEPTH
    mova     m0, [%2+ 0]
    packssdw m0, [%2+32]
    mova     m1, [%2+64]
    ...
    packsswb m0, m1
    PIC_BEGIN r4
    mova     m1, [pic(deinterleave_shufd)]
    PIC_END
    vpermd   m0, m1, m0
%else
    mova     m0, [%2+ 0]
...
%if mmsize==16
    movh      m3, [pic(dct_coef_shuffle)+r4*8]
    movh      m2, [pic(dct_coef_shuffle)+r2*8]
    mova      m4, [pic(pw_256)]
...
%else ; mmsize==32
    movq     xm2, [pic(dct_coef_shuffle)+r4*8]
    vinserti128 m2, m2, [pic(dct_coef_shuffle)+r2*8], 1
    punpcklbw m2, m2
    por       m2, [pic(pw_256)]
    pshufb    m0, m2
...
%endif
    add     eax, r3d
%else
    movzx   eax, byte [pic(popcnt_table)+r2]
    movh m1, [pic(dct_coef_shuffle)+r2*8]
    punpcklbw m1, m1
    por       m1, [pic(pw_256)]
    pshufb    m0, m1
